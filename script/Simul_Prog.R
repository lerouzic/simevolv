...
launchprogevol = function(myfile="param.txt",...) { #launch the program with a parameter file à mettre entre "" - ca fonctionne
  ...
  theta = gv("FITNESS_OTPIMUM", theparamsare)  ; if (length(theta)<L){ for (i in (length(theta)+1):L){theta[length(theta)+1]=theta[1]}} # on verifie que theta a le bon nombre de parametres, sinon on lui rajoute le 1er parametre le nombre de fois qu'il faut
...
gv=function(input, param2){
  dat=param2
  value = as.numeric(unlist(strsplit(dat[[input]],","))) #on peut pas mettre le $ car iput est une variable de m**de
  return(value)
} #end gv
...
read.param = function(paramfile) { #from Estelle: turn un fichier genre table into a list
	...
	ans = list()
	for (i in seq_along(ss)) { ans[[ss[[i]][1]]] = c(ss[[i]][-1])}
	return(ans)
} #end readparam
...
runevolution = function(N=10, gen=10, L=3, mucis=0.5, mutrans=0.5, a=0.2, pas=1, steps, measure, diag, muteff, ...){ ##N=individus gen=generations L=genes ...qui permettent d'acceder aux parametres des functions downstream
  ...
  if (gv("INIT_CLONAL", read.param("param.txt"))==1){    #~~mettre en init_clonal yesss ##hard
...
#note# S0:#minimum,maximum,median,random_binary,random,basal
  ...
  for (i in 1:steps) {
    S0 = sigma.M2p(((t(t(W)*transeffect)) %*% S0), aam1, l1am1) 	#attention, et pas S0 %*% W
    sto[,i+1] = S0
  }
  ans = list()
...
selectforfitness= function(pop, N=10){ #select one individual proportionally to fitness
  fit=matrix(NA, N, 2)
  for (i in 1:N){
    fit[i,1]=i
